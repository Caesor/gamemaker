...
// const texture = PIXI.Texture.from('assets/grassHalfMid.png');
...
loader.add('atlas', 'assets/basic/atlas.json');
loader.add('button', 'assets/basic/button.png');
loader.add('atlas_image', 'assets/basic/atlas.png');
...
function buildTilemap(frame) {
    ...
    for (var i = 0; i < 7; i++)
        for (var j = 0; j < 5; j++) {
            tilemap.addFrame("assets/basic/grass.png", i * size, j * size);
            if (i % 2 == 1 && j % 2 == 1)
                tilemap.addFrame("assets/basic/tough.png", i * size, j * size);
        }

    // if you are lawful citizen, please use textures from
    var textures = resources.atlas.data.frames;
